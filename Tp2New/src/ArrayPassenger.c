...
void printCoverListPassenger ()
{
	printf("\n" // Portada de la lista de pasajeros
				" -------------------------------------------------------------------------------------------------------------------  \n"
				"| Id   ||    Nombre         ||    Apellido      ||      Precio     ||     C�digo de vuelo    ||   IdTipoPasajero    | \n"
				" -------------------------------------------------------------------------------------------------------------------  \n");
}
...
void printfListados ()
{
	printf("\n0) Volver al menu principal\n"
			...
			"2) Listados de los Pasajeros ordenados alfab�ticamente por (Apellido) y (Tipo de pasajero).\n"
			"3) Listado de los Pasajeros por  (C�digo de vuelo)  y estados de vuelos 'Activo', 'Cancelado' y 'Demorado' \n"
			"\n----------------------------------------------\n");
}
...
int AltaForzada (Passenger* arrayPassengersHardcode, int len,int* idPassengers,FlyCode* arrayFlyCodes, int len_flyCode,int* idFlyCode)
{
	...
	if (arrayPassengersHardcode!=NULL && len>0 && arrayFlyCodes !=NULL && len_flyCode > 0)
	{
		...
		{
			retorno=0;
			pass_HardcodeFlyCodePassenger(arrayFlyCodes, len_flyCode,idFlyCode);
			pass_HardcodePassengers(arrayPassengersHardcode, len,idPassengers);
		...
		if (pass_findEmptySpace(arrayPassengers, len) >=0 || pass_findEmptySpaceFlyCode(listFlyCode, len_flyCode)>=0)
		{
			...
			switch (VereficarRetornoAltaFuncion)
			{
				...
				case -3:
					...
					break;
				case -4:
					printf("\nFall� una funcion 'Setters', vereficar cual funcion y en donde esta el error\n\n");
					break;
			}
			retorno=0; //OK
		}
