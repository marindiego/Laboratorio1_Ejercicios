...
void getString (char cadena[], char mensaje[], int tam ) { // Funcion de Practica, Se rompe facil
...
int myGets( char *cadena, int tamMax){
...
int esTextoBasico(char* cadena){
	...
	for(i=0;cadena[i]!='\0';i++){
		...
		if( isspace(cadena[0]) || cadena[i]<32||(cadena[i]>32&&cadena[i]<65)||(cadena[i]>90&&cadena[i]<97)||(cadena[i]>122&&cadena[i]<164)||cadena[i]>165 || !(isalpha(cadena[i]))){
